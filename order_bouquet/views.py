...
from .models import Bouquet, Category, Client, Consultation, Order


class HomeView(generic.ListView):
    queryset = Bouquet.objects.all().order_by("?")[:3]
    template_name = "order_bouquet/index.html"
    ...
    def post(
        ...
        **kwargs: Any,
        ...
        return render(
            request,
            "order_bouquet/consultation_confirm.html",
            context,
        )
...
class CatalogView(generic.ListView):
    ...
    template_name = "order_bouquet/catalog.html"
...
class QuizView(generic.ListView):
    model = Category
    template_name = "order_bouquet/quiz.html"
    context_object_name = "categories"


class QuizStepView(generic.ListView):
    model = Bouquet
    template_name = "order_bouquet/quiz-step.html"
...
class ResultView(generic.ListView):
    model = Bouquet
    template_name = "order_bouquet/result.html"
...
class ConsultationView(generic.CreateView):
    ...
    model = Consultation
    template_name = "order_bouquet/consultation.html"
...
class OrderView(generic.TemplateView):
    template_name = "order_bouquet/order.html"
...
class OrderStepView(generic.View):
    def post(self, request, *args, **kwargs):
        ...
        min_orders_count = min(
            list(
                User.objects.filter(is_staff=False)
                .annotate(c=Count("courier_orders"))
                .values_list("c", flat=True)
            )
        )
        min_count_orders_courier = (
            User.objects.annotate(c=Count("courier_orders"))
            .filter(is_staff=False, c=min_orders_count)
            .first()
        )
        ...
        return render(self.request, "order_bouquet/order-step.html")


class CardView(generic.DetailView):
    model = Bouquet
    template_name = "order_bouquet/card.html"
...
class AdminOrdersList(LoginRequiredMixin, generic.ListView):
    model = Order
    ordering = ["-pk"]
    template_name = "order_bouquet/admin_order.html"
...
class AdminOrderAdd(LoginRequiredMixin, generic.CreateView):
    ...
    model = Order
    template_name = "order_bouquet/order_edit.html"
...
class AdminConsultationList(LoginRequiredMixin, generic.ListView):
    model = Consultation
    ordering = ["-pk"]
    template_name = "order_bouquet/admin_consultation.html"
    context_object_name = "consultations"
    paginate_by = 10
...
class AdminConsultationEdit(LoginRequiredMixin, generic.UpdateView):
    form_class = ConsultationForm
    model = Consultation
    template_name = "order_bouquet/consultation_edit.html"
